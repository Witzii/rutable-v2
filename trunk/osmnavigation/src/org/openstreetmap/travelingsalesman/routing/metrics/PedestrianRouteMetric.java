 ...
 */
public class PedestrianRouteMetric implements IRoutingMetric {
    ...
    public double getCost(final RoutingStep aSegment) {
      ...
      for (WayNode node : nodes) {
          if (lastNode == null) {
        	  ...
        	  if(Settings.getInstance().get("Tipo", "").equals("WheelChair"))
        	  {
	        	  if(this.getHeight(lastNode) < this.getHeight(node))
	        	  {
	        		  ...
	        		  if(percentage > Double.parseDouble(Settings.getInstance().get("Porcentaje", "12")))
	        		  {
