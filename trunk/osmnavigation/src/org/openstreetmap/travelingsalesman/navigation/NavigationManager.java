...
import org.openstreetmap.travelingsalesman.routing.metrics.BlindRouteMetric;
import org.openstreetmap.travelingsalesman.routing.metrics.IRoutingMetric;
import org.openstreetmap.travelingsalesman.routing.metrics.PedestrianRouteMetric;
...
import org.openstreetmap.travelingsalesman.routing.selectors.Blind;
import org.openstreetmap.travelingsalesman.routing.selectors.Pedestrian;
import org.openstreetmap.travelingsalesman.routing.selectors.WheelChair;
...
/*        //TO DO: take all destinations and the current
    ...
    private class RouteCalculator implements Callable<Route>, IProgressListener {
        ...
        public Route call() throws Exception {
            ...
            Thread.currentThread().setName("Routing");
            try {
         
            	 //IRouter router = Settings.getInstance().getPlugin(IRouter.class, TurnRestrictedAStar.class.getName());
                /*JMG*/
                IRouter router = Settings.getInstance().getPlugin(IRouter.class, MultiTargetDijkstraRouter.class.getName());
                /*JMG*/
                ...
                /*JMG*/
                IRoutingMetric metric = null;
                String profile = Settings.getInstance().get("profile", "normal");
                if(profile.equals("normal"))
                {
                    //metric = new ShortestRouteMetric();
                	metric = new PedestrianRouteMetric();
                	setSelector(new Pedestrian());
                }
                else if(profile.equals("mobil") )
                {
                    metric = new PedestrianRouteMetric();
                    setSelector(new WheelChair());
                }
                else if(profile.equals("blind"))
                {
                    metric = new BlindRouteMetric();
                    setSelector(new Blind());
                }
                else if(profile.equals("custom"))
                {
                    //TODO: El vehiculo depende de si elige escaleras o no, ya se vera
                }
                else
                {
                    metric = Settings.getInstance().getPlugin(IRoutingMetric.class, StaticFastestRouteMetric.class.getName());
                }
                /*JMG*/
